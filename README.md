...
Chameleon is a userspace framework for dynamic and secure application memory transformation. Chameleon leverages existing OS primitives (i.e., `ptrace`, `userfaultfd`) and transforms the target application from outside of the target. We use Chameleon to timely re-randomize the target application stack slots.
...
We also created a VM image for testing Chameleon.

1. Download the [VM image](https://drive.google.com/file/d/1HixBrTivwDulycJ5SxBeSYKymrxVsSJd/view?usp=sharing) (~2.3GB).
...
```
3. Follow the README to run the examples:
```
